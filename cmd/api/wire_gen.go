...
func InitializeApp(appName configs.AppName, pgDbConfig configs.PgDbConfig, logConfig configs.LogConfig) (*app, error) {
	...
	portsrepoRepository := portsrepo.New(logger, database)
	portssrvService := portssrv.New(portsrepoRepository, logger)
	portsHandler := ports.NewHandler(logger, portssrvService)
	portsRoutes := ports.New(portsHandler)
	v1Routes := v1.New(routes, currenciesRoutes, portsRoutes)
